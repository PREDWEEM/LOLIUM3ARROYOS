# -*- coding: utf-8 -*-
# 🌾 PREDWEEM — Calibración por clics (Plotly) — Versión segura sin instalación automática
...
try:
    ...
    Instalala manualmente ejecutando este comando en tu entorno local o en requirements.txt:

    ```bash
    pip install streamlit-plotly-events
    ```

    Si estás en **Streamlit Cloud**, agregá esta línea en tu archivo `requirements.txt`:
    ```
...
# ===== SIDEBAR =====
with st.sidebar:
    st.header("🧭 Ajustes de detección y suavizado")
...
for f in files:
    ...
    if len(sel) == 2:
        ...
        st.info("Hacé 2 clics sobre el gráfico (mín y máx).")
        continue
